...
function InsertRow(dealId, clientName, projectName, projectManager, projectCost) {
    ...
    currentDealId++;
    //myDataTest=localStorage.setItem("myData", JSON.stringify(myData))
    CreateTableFromJSON();

}

function EditDeal() {
   // InsertRow(currentDealId,"start", "start","start",0)
    var projectID = document.getElementById("projecidInput").value;
    var projectCost = document.getElementById("projectCostInput").value;
    
    EditRow(projectID, projectCost)

    document.getElementById("projectidInput").value = "";
    document.getElementById("projectCostInput").value = "";


    InsertRow(currentDealId, "yes", "yes", "yes", 2)
    

}

function EditRow(dealId,  projectCost) {
    //InsertRow(currentDealId,myData[dealId].client_name,myData[dealId].project_name,myData[dealId].project_manager, projectCost);
    myData[dealId].project_cost=projectCost;
